...
import { PREFIX_API_LOAI_DE_ADMIN } from '@/constants/url'
import type {
  PaginationParams,
  DefaultResponse,
  ResponseList,
...
export const getSize = async (id: string) => {
  const res = (await request({
    url: `${PREFIX_API_LOAI_DE_ADMIN}/${id}`,
    method: 'GET'
...
export const modifyStatusSize = async (id: string) => {
  const res = (await request({
    url: `${PREFIX_API_LOAI_DE_ADMIN}/${id}/change-status`,
    method: 'PUT'
  })) as AxiosResponse<DefaultResponse<SizeResponse>>
